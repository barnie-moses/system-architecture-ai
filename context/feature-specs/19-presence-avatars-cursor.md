...
2. Add the participant avatar group inside the editor canvas area.
   ...
   - do not render a second avatar for them from the Liveblocks presence list
...
- Participant avatars appear only inside the editor canvas view.
- The editor home navbar is unchanged.
- The current user is shown with the existing Clerk UserButton only.
- Other active participants render as collaborator avatars.
- More than five collaborators show a +N overflow chip.
- Live cursors render for other participants only.
- Cursor position updates through Liveblocks presence.
- Presence types are defined in `liveblocks.config.ts`.
